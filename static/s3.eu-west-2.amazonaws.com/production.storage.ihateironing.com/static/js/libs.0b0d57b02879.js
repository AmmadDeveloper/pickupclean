...
(function() {
  ...
  // From http://api.jquery.com/jQuery.Callbacks/#pubsub
...
(function($) {
  ...
  $.fn.scrollfade = function() {
    ...
    elems.addClass(options.className);

    var navbarHeight = $('.navbar').first().outerHeight() || 0;
    ...
    function update() {
      for (var i = 0; i < numElems; i++) {
        ...
        var triggerY = rect.top + elem.offsetHeight / 2;
        if (triggerY >= navbarHeight && triggerY <= window.innerHeight) {
          elem.classList.add(options.fadeInClassName);
        } else {
          elem.classList.remove(options.fadeInClassName);
        }
      }
    }
...
window.IronWeb.Nav = (function() {
  'use strict';

  var MENU_ITEM = '.navbar-ihi li';
  var MENU_ITEM_TEXT = MENU_ITEM + ' a span';

  var stickyNav = false;
  var navbarTogglePoint = 5;

  var body = $(document.body);
  var navbar = $('header .navbar');
  var mobileNavbarToggle = $('header .navbar-toggle');
  var logoContainer = $('.navbar-brand.logo-container');

  var classes = {
    ENFORCE_WHITE: 'enforce-white',
  ...
  function initBurgerNav() {
    mobileNavbarToggle.on('click touch', function() {
      body.toggleClass(classes.NO_SCROLL);
      $(this).toggleClass(classes.OPEN);
      logoContainer.toggleClass(classes.ENFORCE_WHITE);
    });
  }

  function handleScroll() {
    var scrollTop = $(window).scrollTop();
    navbar.toggleClass('scrolled', scrollTop > navbarTogglePoint);
  }

  function initSmoothScrolling() {
    var navbarHeight = stickyNav ? navbar.outerHeight() : 0;

    $('a.btn-cta').click(function(event) {
      if (
...
$(document).ready(function() {
  'use strict';
  window.IronWeb.Nav.init();
});

window.IronWeb = window.IronWeb || {};

window.IronWeb.Utils = (function() {
  function addSelectWrapper(element, useLightStyle) {
    var wrapper = document.createElement('div');
    wrapper.className = useLightStyle
      ? 'select-wrapper select-wrapper--light'
      : 'select-wrapper';
    element.parentNode.insertBefore(wrapper, element);
    wrapper.appendChild(element);
  }

  // Wrap select elements so custom caret gets applied
  function styliseSelects(selector, useLightStyle) {
    var selectElements = document.querySelectorAll(selector);
    for (var i = 0; i < selectElements.length; i++) {
      addSelectWrapper(selectElements[i], useLightStyle);
    }
  }

  return {
    styliseSelects: styliseSelects,
  };
})();
